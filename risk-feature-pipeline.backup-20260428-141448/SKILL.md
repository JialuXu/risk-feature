...
```bash
cd "<pipeline_root>"
python3 -m shared --pipeline credit                          # 征信全流程
python3 -m shared --pipeline gsfc                           # 工商财务全流程
python3 -m shared --pipeline generic \
  --input data/processed/<宽表>.csv --target is_bad \
  --project-name <项目名>                                   # generic 全流程
python3 -m shared --pipeline generic \
  --input data/processed/<宽表>.csv --target is_bad \
  --merge-target data/raw/<坏客户清单>.csv \
  --project-name <项目名>                                   # 合并坏客户清单
```

> 单维快路径推荐直接调用 `run_generic_pipeline()`（Python API），因 CLI 暂无 `--category-dims` 等参数。模板见 `AGENTS.md`。
