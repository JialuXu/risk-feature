...
def prepare_credit_wide_table(data):
    ...
    if all(c in df_cust.columns for c in _amt_cols):
        for c in _amt_cols:
            df_cust[c] = np.where(df_cust[c] == '-', np.nan, df_cust[c])
            df_cust[c] = (
    ...
    ) + tuple(COL_SEGMENT_DIMS) + tuple(COL_INDUSTRY_DATA_COLS) + (
        '行业大类', '行业小类',
    )
    ...
    target_cols = [c for c in numeric_cols if c not in (COL_CUSTOMER_ID, COL_TARGET)]
    
    # 批量找出有缺失值的列
    missing_mask = df_base[target_cols].isna().sum() > 0
    missing_cols = missing_mask[missing_mask].index.tolist()
    
    fill_count = 0
    if missing_cols:
        fill_count = df_base[missing_cols].isna().sum().sum()
...
def _winsorize_series(series, limits=(0.01, 0.09)):
    """对比率特征进行1%和99%的缩尾处理"""
    if len(series) == 0:
        return series
    # 仅对有效值进行分位数计算
...
def create_credit_features(df):
    ...
    2. 提取行为逻辑，反映融资行为模式
    3. 避免除零错误，使用安全除法
    4. 对衍生的比值特征进行 1%/99% 缩尾处理，防范极值
    """
    df = df.copy()

    # -- 结构占比类 --
    df['非银机构占比'] = _winsorize_series(safe_divide(df['非银授信机构数'], df['总机构数']))
    df['银行机构占比'] = _winsorize_series(safe_divide(df['银行授信机构数'], df['总机构数']))
    df['小贷公司占比'] = _winsorize_series(safe_divide(df['小额贷款公司数'], df['总机构数']))
    df['消金公司占比'] = _winsorize_series(safe_divide(df['消费金融公司数'], df['总机构数']))
    df['未结清机构占比'] = _winsorize_series(safe_divide(df['信贷交易未结清总机构数'], df['总机构数']))

    # -- 渠道风险特征 --
    df['高风险渠道数'] = df['小额贷款公司数'] + df['消费金融公司数']
    df['高风险渠道占比'] = _winsorize_series(safe_divide(df['高风险渠道数'], df['总机构数']))
    df['是否有高风险渠道'] = (df['高风险渠道数'] > 0).astype(int)
    df['非银银行比例'] = _winsorize_series(safe_divide(df['非银授信机构数'], df['银行授信机构数']))

    # -- 行为指标类 --
    df['授信分散度'] = np.log1p(df['总机构数'])

    # -- 效率比值类（核心衍生指标）--
    df['担保查询未结清比'] = _winsorize_series(safe_divide(df['担保人征信查询次数'], df['信贷交易未结清总机构数']))
    df['担保查询机构比'] = _winsorize_series(safe_divide(df['担保人征信查询次数'], df['总机构数']))
...
def compare_corr_lr(corr_df, lr_coef_df):
    ...
    choices = ['稳健特征', '冗余特征', '抑制变量']
    
    comp['特征类型'] = np.select(conditions, choices, default='弱相关')
    ...
    for gname, gdf in df_v.groupby(dim_col):
        ...
        for feat_set, label in [(raw_features, 'raw'), (derived_features, 'derived')]:
            ...
            try:
                ...
                auc_val, auc_tp = eval_lr_roc_auc(
                    mdl, Xs, y, n_total, n_bad, _T
                )
                if label == 'raw':
                    auc_raw = auc_val
                    auc_type_raw = auc_tp
        ...
        rows.append({
            ...
            'AUC类型_原始特征': auc_type_raw,
            'AUC类型_衍生特征': auc_type_derived,
            'AUC差异': diff,
        })
...
def export_results(project_root, results):
    """
    导出全部分析结果到CSV
    ...
    返回:
        exported: 导出的文件列表
    """
    pname = CREDIT_CONFIG['project_name']
    out_dir = os.path.join(project_root, OUTPUT_DIR_CREDIT)
    res_dir = os.path.join(project_root, RESULTS_DIR_CREDIT)
    ensure_dir(out_dir)
    ensure_dir(res_dir)
    ...
    # 1. 全量IV分析结果
    _save(results.get('iv_full'), f'{pname}_IV分析结果.csv')
    ...
    # 6. 分群样本概况
    _save(results.get('reliability_summary'), f'{pname}_分群样本概况.csv')

    # 7. 原始vs衍生特征AUC对比
    comp_all = results.get('feature_set_comparison')
    _save(comp_all, f'{pname}_原始vs衍生特征AUC对比.csv')

    # 8. 综合特征分析结果（输出到output/）
    _save(results.get('comprehensive'), f'{pname}_综合特征分析结果.csv', out_dir)

    # 9. LLM报告数据（JSON + CSV，输出到output/）
    llm_data = results.get('llm_report_data')
    if llm_data is not None:
        try:
            json_path = _export_llm_report_json(project_root, llm_data)
            ...
            if feat_df is not None and not feat_df.empty:
                _save(feat_df, f'{pname}_LLM_特征有效性汇总.csv', out_dir)
            seg_df = llm_data.get('segment_profiles')
            if seg_df is not None and not seg_df.empty:
                _save(seg_df, f'{pname}_LLM_分群画像.csv', out_dir)
        except Exception as e:
...
def build_comprehensive_table(iv_full_df, corr_results, lr_coef_results,
    ...
    df = iv_full_df[['特征名称', '特征类型', 'IV值', '预测能力', 'IV可信度']].copy()
    df = df.rename(columns={'IV值': 'iv_all'})

    # 添加全量相关系数和P值
    # (这里简化处理，使用第一个可用的相关系数结果)
    for dim_name, corr_df in corr_results.items():
    ...
    if iv_group_all is not None and not iv_group_all.empty:
        for dim_name in iv_group_all['分群维度'].unique():
            ...
            iv_mean = dim_iv.groupby('特征')['IV值'].mean()
...
def _export_llm_report_json(project_root, llm_data):
    """
    ...
    # 4c. 分级：重点 vs 简略
    简略字段 = {'分群维度', '分群名称', '样本数', '坏客户数', '坏客户率', '风险特征概要'}
    重点分群 = []
    简略分群 = []
