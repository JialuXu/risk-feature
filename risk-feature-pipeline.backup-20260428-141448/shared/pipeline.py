...
---------
# 方式1：Python API
>>> from shared.pipeline import run_credit_pipeline, run_gsfc_pipeline
>>> results = run_credit_pipeline()
>>> results = run_gsfc_pipeline()

# 方式2：命令行
$ cd risk-feature-pipeline && python -m shared.pipeline --pipeline credit
$ cd risk-feature-pipeline && python -m shared.pipeline --pipeline gsfc
$ cd risk-feature-pipeline && python -m shared.pipeline --pipeline gsfc --steps data_prep,feature_eng,univariate
"""
