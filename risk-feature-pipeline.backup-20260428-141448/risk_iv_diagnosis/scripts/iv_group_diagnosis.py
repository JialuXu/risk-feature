...
def prepare_credit_wide_table(data):
    ...
    if '产业数据' in data:
        ...
        for c in COL_INDUSTRY_DATA_COLS:
            if c in df_ind.columns:
                ind_cols.append(c)
        # 二进制资质标签列（通过 ColumnMapper 配置的前缀检测）
        qual_cols = _mapper.detect_qual_cols(df_ind.columns)
    ...
    for col in df_base.columns:
        if df_base[col].dtype == object and col not in _mapper.numeric_skip_cols:
    ...
    numeric_cols = [c for c in numeric_cols if c not in [COL_CUSTOMER_ID, COL_TARGET]]
...
def create_credit_features(df):
    ...
    # -- 结构占比类 --
    df['非银机构占比'] = winsorize_series(safe_divide(df['非银授信机构数'], df['总机构数']))
    df['银行机构占比'] = winsorize_series(safe_divide(df['银行授信机构数'], df['总机构数']))
    df['小贷公司占比'] = winsorize_series(safe_divide(df['小额贷款公司数'], df['总机构数']))
    df['消金公司占比'] = winsorize_series(safe_divide(df['消费金融公司数'], df['总机构数']))
    df['未结清机构占比'] = winsorize_series(safe_divide(df['信贷交易未结清总机构数'], df['总机构数']))

    # -- 渠道风险特征 --
    df['高风险渠道数'] = df['小额贷款公司数'] + df['消费金融公司数']
    df['高风险渠道占比'] = winsorize_series(safe_divide(df['高风险渠道数'], df['总机构数']))
    df['是否有高风险渠道'] = (df['高风险渠道数'] > 0).astype(int)
    df['非银银行比例'] = winsorize_series(safe_divide(df['非银授信机构数'], df['银行授信机构数']))
    ...
    df['担保查询未结清比'] = winsorize_series(safe_divide(df['担保人征信查询次数'], df['信贷交易未结清总机构数']))
    df['担保查询机构比'] = winsorize_series(safe_divide(df['担保人征信查询次数'], df['总机构数']))
