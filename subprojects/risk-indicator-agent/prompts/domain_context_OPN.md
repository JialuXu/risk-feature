...
OPN 域聚焦工商变更行为模式 (法人变更频率 / 股东变化 / 地址迁移 等). **本轮挖掘 IV 中等 (0.14–0.28)**, 覆盖率高.

## 主要基础表

- `DT_SSDP_CORP_CUST_BASIC_INFO_W` —— 基础信息 (`FOUND_DT`=成立日期 / `IS_TECH_CORP`=是否科技 / 注册资本)
- `DT_SSDP_CORP_GS_CHG_INFO_A` —— **【缺口表,数仓待补建】** 工商变更明细
   - 必备字段: CUST_NO, CHG_EVENT, CHG_BEFORE, CHG_AFTER, CHG_DT
   - 推荐字段: CHG_TYPE_STD (18 类标准事项), CHG_AMT_BEFORE, CHG_AMT_AFTER
- `DT_SSDP_CORP_GS_TAG_INFO_A` —— **【缺口表】** 工商标记 (新增/退出类)
...
- **大部分 OPN 候选的 priority 应是 P0-阻塞**, 因为基础表 `DT_SSDP_CORP_GS_CHG_INFO_A` 当前未建. 在 notes 写 "依赖工商变更明细表, 待数仓补建后落地"
