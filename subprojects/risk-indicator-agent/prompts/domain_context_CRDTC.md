...
征信域指标来自人行征信报告. 通常是信号较强的维度.

## 主要基础表

- `DEMO_CORP_CREDIT_REPORT` —— 对公客户征信信息 (核心: 担保人查询次数 / 总机构数 / 各类未结清明细)
- `DEMO_CORP_CREDIT_REPORT_EXT` —— 对公客户其他征信指标 (信贷交易未结清总机构数 / 银行授信机构数 / 非银授信机构数 / 当前逾期机构数 / 借贷余额)
...
- 银行授信机构数 → 通常大者风险低 (银行多代表信誉好), 但特定客群可能呈现"正向", 以 seed 给出的 risk_direction 为准
- 当前逾期余额 / 当前逾期机构数 → 强正向

## 注意事项

- 部分原始字段口径需要核实 (如 GUAR_QRY_CNT 是累计还是滚动 24 月) — 在 notes 写 "口径需向数源核实是累计还是窗口"
