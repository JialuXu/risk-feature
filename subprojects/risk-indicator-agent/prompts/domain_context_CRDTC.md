...
具体表名以下方「可用基础表清单」为准, 按内容大致分两类:

- 征信主表 —— 对公客户征信信息 (核心: 担保人查询次数 / 总机构数 / 各类未结清明细)
- 征信补充表 —— 对公客户其他征信指标 (信贷交易未结清总机构数 / 银行授信机构数 / 非银授信机构数 / 当前逾期机构数 / 借贷余额)
...
- 部分原始字段口径需要核实 (如担保人查询次数是累计还是滚动 24 月) — 在 notes 写 "口径需向数源核实是累计还是窗口"
