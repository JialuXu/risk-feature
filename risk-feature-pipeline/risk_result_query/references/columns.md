...
> 对外列名为 `特征` / `分群维度` / `分群名称`；文件为 `_IV分析结果_全量.csv` / `_IV分析结果_分群.csv`。
...
| `IV_缺失贡献` / `IV_非缺失贡献` | float | 缺失箱与非缺失部分各自的 IV 贡献 |
| `缺失样本数` | int | |
| `分群总样本数` / `分群坏客户数` / `分群坏客户率` | int / int / float | 全量样本口径 |
| `特征有效样本数` / `特征坏客户数` | int | 该特征参与计算的样本 |
| `实际分箱数` | int | 自适应分箱后的箱数 |
| `IV可信度` | str | 可信 / 参考 / 不可信-样本不足 / 不可信-疑似数据穿越 |

落盘顺序即特征顺序，未排序；`top_features(..., kind='iv')` 会按 `IV值` 降序取 top-N。

> **`预测能力` 列在 `r.comprehensive`**（由 `iv_all` 衍生），`iv_full` 与 `top_features(..., kind='iv')` 的结果里没有这一列。
