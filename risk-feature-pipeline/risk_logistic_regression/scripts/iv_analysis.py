...
def calc_iv_simple(df, feature, target, bins=10):
    """
    简化接口：只返回 IV 值（兼容旧代码调用）
    """
    iv, _ = calc_iv(df, feature, target, bins=bins)
    return iv


def _run_segment_iv(df_sub, feature_cols, group_name, results):
    """
    对指定子集计算所有特征的 IV 值，并附带样本元信息和可信度评估。
    内部辅助函数，避免重复代码。
    """
    n_total = len(df_sub)
    n_bad_total = int(df_sub[COL_TARGET].sum())
    bad_rate = n_bad_total / n_total if n_total > 0 else 0.0

    for feat in feature_cols:
        iv, meta = calc_iv(df_sub, feat, COL_TARGET)
...
def run_iv_analysis(df, feature_cols):
    ...
    # 全量 IV
    print("[INFO] 计算全量 IV...")
    _run_segment_iv(df, feature_cols, '全量', results)
    ...
    for dim_key, dim_col in COL_SEGMENT_DIMS_DICT.items():
        ...
        for val in unique_vals:
            ...
            if len(df_sub) >= MIN_SAMPLES and df_sub[COL_TARGET].sum() >= MIN_BAD_SAMPLES:
                _run_segment_iv(df_sub, feature_cols, f'{display}_{val}', results)

    # 腰部企业（中等授信+高内评等级）
    if '是否腰部企业' in df.columns:
        print("[INFO] 腰部企业 IV 计算...")
        df_waist = df[df['是否腰部企业'] == 1]
        if len(df_waist) >= MIN_SAMPLES and df_waist[COL_TARGET].sum() >= MIN_BAD_SAMPLES:
            _run_segment_iv(df_waist, feature_cols, '腰部企业', results)

    # 中等授信非高评级
    if '是否中等授信非高评级' in df.columns:
        print("[INFO] 中等授信非高评级 IV 计算...")
        df_mid_nonhigh = df[df['是否中等授信非高评级'] == 1]
        if len(df_mid_nonhigh) >= MIN_SAMPLES and df_mid_nonhigh[COL_TARGET].sum() >= MIN_BAD_SAMPLES:
            _run_segment_iv(df_mid_nonhigh, feature_cols, '中等授信非高评级', results)
