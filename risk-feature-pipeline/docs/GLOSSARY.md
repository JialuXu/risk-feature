...
**约定**：
- 在 CSV 中列名固定为 `分群名称`（旧名 `分群值` 仅在历史 CSV 中存在）
...
**面向业务用户解释判定结果时，统一使用本节口径与数值**：不向业务用户展示 `MIN_BAD_LR` 等英文常量名。数值与 `risk_core/config/default.yaml` 同步维护——改 YAML 时必须同步更新本节。
...
|---|---|---|
| `5折交叉验证` | 总样本 ≥ 200 且 坏客户 ≥ 30 | "经过交叉验证，这个数字可靠" |
...
| 规则稳定性 | 留出集 bootstrap 重抽样（默认 200 次）：有效次数占比 < 0.6 → 不稳定；否则坏账率变异系数 ≤ 20% 稳定 / ≤ 40% 较稳定 / > 40% 不稳定 | "「不稳定」的规则换批数据可能就不成立，**不能直接写进政策**，须人工复核" |
