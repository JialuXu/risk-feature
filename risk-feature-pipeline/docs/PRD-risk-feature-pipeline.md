...
| 产物 | 形态 | 用途 |
|---|---|---|
| 相关系数表 | DataFrame（长格式） | 喂给 `compare_corr_lr` 与导出 |
...
4. **IV 预测能力分档**：
   ...
   - **过拟合嫌疑**：IV > 2.0 直接标黄，**不进入推荐特征**。
5. **可信度评级**：综合样本量 + IV 大小判断：
   - 可信 / 参考 / 不可信-样本不足 / 不可信-过拟合嫌疑。
...
2. **标准化 + L2**：`StandardScaler` + `LogisticRegression(penalty='l2', C=1.0, solver='lbfgs')`，系数以**标准化后尺度**解释。
3. **AUC 类型自动选择**：
...
5. **相关 vs LR 对照**（`compare_corr_lr`）：把单变量相关系数与多变量 LR 系数对比，给每个特征贴标签：
   - **稳健**：相关与系数同向且都显著。
   - **冗余**：相关显著但系数被压成 0（被其他特征替代）。
   - **抑制**：相关与系数异向（共线性扭曲）。
   - **弱相关**：两边都不显著。
...
| 相关-LR 对照表 | DataFrame | 流入 LLM 报告数据 |
...
2. **特征预筛**：用 IV ≥ `IV_THRESHOLD['medium']` 且可信度可信/参考的特征作为树输入；剔除 IV > `IV_SUSPECT_THRESHOLD`（2.0）的过拟合嫌疑特征。
3. **决策树拟合**：
   - `max_depth = 3`（限制规则可读性）。
...
4. **构建分群画像 CSV**：每分群一行，含样本数 / 坏率 / 模型 AUC / Top3_IV特征 / Top3_风险相关特征。
