...
> 验证日期：2026-05-12  
> 数据：`<仓库根>/data/raw/`（舆情特征宽表 ⨝ 客户信息含评级；具体规模已脱敏）  
> 平台：macOS 14（Darwin 25.4.0）+ Python 3.12.13

## ✅ 修复状态（2026-05-12 第二轮）

所有 4 个 bug + 2 个语义改进**已修复**。下方各项前的 ✅ 表示已落地。详见 `<本地修复方案>` 修复方案与各文件 diff。
...
- **影响**：下游脚本若用 `df['不通过原因'].str.contains(...)` 会爆 `AttributeError`，但目前管线内部没有这类调用。
- **优先级**：低
...
- **业务侧**：这条规则其实风险倍数高、显著性强，非常有效，却被门槛误杀。
