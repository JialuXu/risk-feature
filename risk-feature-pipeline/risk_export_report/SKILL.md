...
| `_IV值分析.csv` | 分群 IV 明细 + 可信度 |
| `_IV值透视表.csv` | 分群×特征 IV 透视 |
...
> 完整列名见 `risk_result_query/references/columns.md`（同时适用于磁盘 CSV 与内存 DataFrame）。

**`_IV值分析.csv`**：`分群维度` / `分群名称` / `特征` / `IV值` / `IV可信度` / `样本数` / `坏客户数`

**`_逻辑回归系数.csv`**：`分群维度` / `分群名称` / `系数`（标准化后，**不是** `LR系数`）/ `AUC` / `AUC类型`

**`_综合特征分析结果.csv`**：`特征名称`（**不是** `特征`）/ `iv_all`（**不是** `IV值`）/ `IV可信度` / `预测能力`
