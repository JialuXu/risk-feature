...
- **推荐调用方式**：通过 `shared/pipeline.py` 的 `run_generic_pipeline(..., steps=[..., 'export'])` 触发，不建议直接调用脚本（避免路径参数不一致）。
...
| 文件后缀 | 内容 |
|----------|------|
| `_IV分析结果_全量.csv` ⭐ | 全量 IV（A5 新名；旧名 `_IV分析结果.csv` 仍写一份兼容副本，下版本移除） |
| `_IV分析结果_分群.csv` ⭐ | 分群 IV 明细 + 可信度（A5 新名；旧名 `_IV值分析.csv` 仍写一份兼容副本） |
...
| `_audit.json` ⭐ | 机器可读自检：IV>2 过拟合特征、不稳定规则、Level 状态（C12） |
...
> A4 后对外列名已统一为「特征 / 分群维度 / 分群名称」三件套；详见 `docs/SCHEMA.md` 与 `risk_result_query/references/columns.md`（同时适用于磁盘 CSV 与内存 DataFrame）。
...
**`_综合特征分析结果.csv`**：`特征` / `iv_all`（**不是** `IV值`）/ `IV可信度` / `预测能力`（A4 后特征列已统一为 `特征`，旧 CSV 中可能仍是 `特征名称`，`load_results()` 自动 rename）
